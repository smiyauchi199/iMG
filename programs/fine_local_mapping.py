...
def patch_means_thetas(patch_pis, patch_uvs, patch_idx, patch_tx, phi, x, weights, output_folder):
    ...
    pcu.write_obj(output_folder + "/" + "updated_input_points_in_third_mapping.obj", mean_pts.cpu().detach().numpy(), np.array([], dtype='int'), np.array([], dtype='float32'))
...
def patch_means_sampled(patch_sampled_xs, phi, patch_sampled_uvs):
    num_patches = len(patch_sampled_uvs)

    mean_pts = patch_sampled_xs

    for i in range(num_patches):

        uv_i = patch_sampled_uvs[i]
        y_i = phi[i](uv_i).squeeze()

        mean_pts[i] = 0.5*mean_pts[i] + 0.5*y_i
...
def transform_mesh_based_on_points(x, transform):
    translate, scale, rotate = transform
    translate = translate.cpu().numpy()
    scale = scale.cpu().numpy()
    rotate = rotate.cpu().numpy()
    ...
    translate2 = -np.mean(x_tx, axis=0)
    scale2 = np.array([1.0 / np.max(np.linalg.norm(x_tx + translate2, axis=1))], dtype=np.float32)
    x_tx = scale2 * (x_tx.astype(np.float32) + translate2)
...
def save_input_points(output_folder, x_for_each_label, transform, device):
    ...
    for i, x in enumerate(x_for_each_label):
        ...
        pcu.write_obj(output_folder + "/" + "input_part_of_sphere_{}.obj".format(i), x_t, np.array([], dtype='int'), np.array([], dtype='float32'))

def save_input_points_without_transform(name, output_folder, x_for_each_label, device):

    for i, x in enumerate(x_for_each_label):
        pcu.write_obj(output_folder + "/" + "{}_input_part_of_sphere_{}.obj".format(name, i), x, np.array([], dtype='int'), np.array([], dtype='float32'))
...
def third_mapping_main(filename, s_filename, nl, ng, d, o_filename, o_folder, intrplt, weight_ratio, specific_scale):
    ...
    anchor = 'overlapping_ellipses_after_2nd_mapping'
    label_for_input = np.loadtxt(o_folder + '/input_label_{}.txt'.format(anchor))
    label_for_input = np.array(label_for_input, dtype='int')

    #Load weights for integrating local meshes
    thetas = np.loadtxt(o_folder + '/angles_{}.txt'.format(anchor))
    thetas = np.array(thetas, dtype='float')

    input_thetas = np.loadtxt(o_folder + '/input_angles_{}.txt'.format(anchor))
    input_thetas = np.array(input_thetas, dtype='float')

    sampled_input_points_in_each_region = load_list(o_folder + '/sampled_input_points_in_each_region_{}.txt'.format(anchor))

    f = []
    n = []
    num_of_regions = label_for_input[-1, 1]+1
    ...
    v_sphere_for_each_region, f_sphere_for_each_region, _, ids_sphere_table, theta_weights, v_sphere_for_each_region_tmp, f_sphere_for_each_region_tmp = utils_overlapping.select_points_corresponding_to_label(v_sphere, f_sphere, n_sphere, label_for_sphere, num_of_regions, 'sphere', thetas)

    f_sphere_for_each_region = f_sphere_for_each_region_tmp.copy()
    for i, s_e in enumerate(v_sphere_for_each_region_tmp):
    ...
    init_original_pis = []
    init_sampled_pis = []
    all_sampled_inputs = np.zeros([v_sphere.shape[0], 3], dtype='float32')
    counter = np.zeros([v_sphere.shape[0]], dtype='int32')
    ...
    for region in range(num_of_regions):
        ...
        uv = torch.from_numpy(v).to(args.devices[0])

        patch_original_xs.append(torch.from_numpy(x_for_each_label[region]).to(args.devices[0]))
        patch_sampled_xs.append(torch.from_numpy(transform_mesh(sampled_input_points_in_each_region[region], transform_for_each_region[region])).to(args.devices[0]))
        ps_uv = torch.from_numpy(v_sphere_for_each_region[region]).to(args.devices[0])
        patch_idx.append(torch.from_numpy(ids_table[region]).to(args.devices[0]))
        _, p = sinkhorn_loss(patch_original_xs[region].unsqueeze(0), uv.unsqueeze(0))
        _, p_sampled = sinkhorn_loss(patch_sampled_xs[region].unsqueeze(0), ps_uv.unsqueeze(0))

        uv = torch.from_numpy(transform_mesh_scale(v, specific_scale)).to(args.devices[0])
        patch_original_vt.append(uv)
        ps_uv = torch.from_numpy(transform_mesh_scale(v_sphere_for_each_region[region], specific_scale)).to(args.devices[0])
        patch_sampled_vt.append(ps_uv)

        init_original_pi = p.squeeze().max(0)[1]
        init_sampled_pi = p_sampled.squeeze().max(0)[1]
        
        init_original_pis.append(init_original_pi)
        # Adjustment of vertex IDs
        init_sampled_pis.append(init_sampled_pi+len(init_original_pi))
        
    all_sampled_inputs = torch.from_numpy(all_sampled_inputs).to(args.devices[0])
    ...
    init_pis_nums = []
    counter = 0
    num_of_patch_original_xs = 0
    num_of_patch_sampled_xs = 0
    for pox, psx, pov, psv, iop, isp in zip(patch_original_xs, patch_sampled_xs, patch_original_vt, patch_sampled_vt, init_original_pis, init_sampled_pis):
        patch_xs.append(torch.tensor(torch.cat([pox, psx], dim=0), device=args.devices[0]))
        patch_uvs.append(torch.tensor(torch.cat([pov, psv], dim=0), requires_grad=True, device=args.devices[0]))
        init_pis.append(torch.cat([iop, isp], dim=0))
        init_pis_nums.append([iop.size(), isp.size()])

        counter += 1
        num_of_patch_original_xs += pox.size()[0]
        num_of_patch_sampled_xs += psx.size()[0]
    ...
    for epoch in range(args.local_epochs):
        ...
        for i in range(num_patches):
            ...
            with torch.no_grad():
                ...
                pi_i = init_pis[i]
                pi_i_nums = init_pis_nums[i][0]
                pi[i] = pi_i
                
            # Weight of the loss function
            weight = weight_ratio*(num_of_patch_original_xs / num_of_patch_sampled_xs)
            loss_i = mse_loss(x_i[pi_i[0:pi_i_nums[0]]].unsqueeze(0), y_i[0:pi_i_nums[0]].unsqueeze(0)) + weight*mse_loss(x_i[pi_i[pi_i_nums[0]:]].unsqueeze(0), y_i[pi_i_nums[0]:].unsqueeze(0))
    ...
    n = utils_overlapping.write_ori(args.output + "_local.ori", v_sphere, f_sphere)
    pcu.write_obj(args.output + "_local.obj", v_sphere, f_sphere, n)
    ...
    if args.interpolate:
        ...
        with torch.no_grad():
            ...
            patch_original_xs = patch_means_thetas(init_original_pis, patch_original_vt, patch_idx, transform_for_each_region, phi, x, input_theta_weights, o_folder)

            patch_xs = []
            patch_uvs = []
            for pox, pov in zip(patch_original_xs, patch_original_vt):
                patch_xs.append(torch.tensor(pox, device=args.devices[0]))
                patch_uvs.append(torch.tensor(pov, requires_grad=True, device=args.devices[0]))
